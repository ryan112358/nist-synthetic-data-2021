...
def run_mechanism(df, schema, run):
    ...
    submit['epsilon'] = epsilon
    cols = ['epsilon', 'taxi_id', 'shift', 'company_id', 'pickup_community_area', 'dropoff_community_area', 'payment_type', 'fare', 'tips', 'trip_total', 'trip_seconds', 'trip_miles']
    submit = submit[cols]
    print('Done with Epsilon=%.1f' % epsilon)
    return submit
...
def main(
    ...
    taxi_id_schema = schema.pop('taxi_id')
    del schema['trip_day_of_week']
    del schema['trip_hour_of_day']
    df = pd.read_csv(ground_truth_file)

    foo = partial(run_mechanism, df, schema)
    ans = map(foo, params['runs'])
    ans = pd.concat(ans)
    ans['taxi_id'] += taxi_id_schema['min']
